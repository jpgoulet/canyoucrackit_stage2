...
class Memory(object):
   ...
   def read(self, segment, offset):
      index = segment * segment_size + offset
      if index >= 0 and index < len(self.data):
         return self.data[index]
      else:
         raise VMException("Invalid read memory access")
   ...
   def write(self, segment, offset, data):
      ...
      if index >= 0 and index < len(self.data):
         ...
         raise VMException("Invalid write memory access")
         
   def dump(self): 
      """Dump memory in hexedit format"""  
      print "********** Memory **********"   
      for index in range(0, len(self.data)/segment_size):
         #segment index
         print "0x{0:02X}: ".format(index),
         
         #hexadecimal value
         values = self.data[index * segment_size: (index + 1) * segment_size]
         for value in values:
            print "{0:02X}".format(value),
